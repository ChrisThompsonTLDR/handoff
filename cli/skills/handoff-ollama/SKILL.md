---
name: handoff-ollama
description: 把执行性编码/调查任务整包交给 Ollama 后台执行（本地模型，低成本）。后台运行，完成后自动通知。支持并行多任务，支持续接（resume）上次会话继续派发后续任务。
---

# handoff-ollama Skill
...
```bash
handoff run --backend ollama --slug <≤3个英文单词的任务助记词> - <<'__HF_EOF__'
[prompt 内容]
__HF_EOF__
```
...
启动后从 **stdout** 捕获唯一有用的一行 `RESULT=<任务路径>`（如 `~/.handoff/tasks/0613-ol-03-fix-auth.result.md`），缩写成 `~/.handoff/...` 后回显给用户。**这条路径的文件名主干（去掉目录和 `.result.md` 后缀）就是本次 run_id**（上例 → `0613-ol-03-fix-auth`）；记住它，用户要求"继续上次/接着再做 X"时靠它 `resume`。
...
```bash
handoff resume <run_id> --backend ollama --slug <任务助记词> - <<'__HF_EOF__'
[后续任务内容]
__HF_EOF__
```
